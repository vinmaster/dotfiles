...
alias project='cd $(find ~/work -maxdepth 1 -type d | fzf)'
...
my() {
  local file="$HOME/code/my"
  openssl aes-256-cbc -a -d -in "$file.enc"
}
myfind() {
  my | ag -A 4 $1;
}
myupdate() {
  local file="$HOME/code/my"
  openssl aes-256-cbc -a -d -in "$file.enc" -out "$file.txt"
  vi "$file.txt"
  echo "EDIT MODE. PRESS ENTER TO CONTINUE";
  read -s;
  openssl aes-256-cbc -salt -a -e -in "$file.txt" -out "$file.enc"
  rm "$file.txt";
  ls $file.*;
}
findfile() { find . | ag $1; }
...
hist() { history | tail -r -n 20 | awk '{$1 = ""; print $0;}' | awk '{$1=$1};1' | fzf | bash; }
